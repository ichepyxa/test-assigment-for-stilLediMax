...
/**
 * Класс User.
 * Класс предназначен для работы пользователя с базой данных. Класс осуществляет сохранение, удаление, форматирование пользователя, преобразование пола из двоичной системы в строку и преобразование даты рождения в количество полных лет.
 */
class User 
{
  protected int $id;
  ...
  function __construct(int $id, string $name = '', string $surname = '',
    ...
    if ($result) {
      ...
      echo "Пользователь с идентификатором: {$result['id']} - есть в базе данных<br>";
      return;
    }
    ...
    $this->dateOfBirth = $dateOfBirth === null ? new DateTime('now') : $dateOfBirth;
  ...
  public function save(): void
  {
    try {
      ...
      echo "Успешное сохранение пользователя<br>";
  ...
  /**
   * Функция удаления пользователя из базы данных по полю id.
   * 
   * @return void
   */
  public function delete(): void
  {
    try {
      ...
      echo "Успешное удаление пользователя с id = {$this->id}<br>";
    } catch (PDOException $e) {
  ...
  /**
   * Функция форматирования пользователя с преобразованием возраста и даты рождения.
   * 
   * @return stdClass
   */
  ...
  /**
   * Функция преобразования пола из двоичной системы в строку.
   * 
   * @param int $gender
   * @return string
   */
  ...
  /**
   * Функция преобразования даты рождения в количество полных лет.
   * 
   * @param DateTime $dateOfBirth
   * @return int
   */
